	...
	<div class="app-contain" :style='{"padding":"30px 10% 30px","margin":"0px auto","alignItems":"flex-start","color":"#666","flexWrap":"wrap","display":"flex","width":"100%","fontSize":"14px","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">营业时间</div>
					<div  class="info_text" >{{detail.yingyeshijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">店内设施</div>
					<div  class="info_text" >{{detail.dianneisheshi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">商家简介</div>
					<div  class="info_text" >{{detail.shangjiajianjie}}</div>
				</div>
				<div class="btn_view">
					...
					<el-button class="edit_btn" v-if="centerType&&btnAuth('shangjiaxinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('shangjiaxinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="商家公告" name="first">
				<div v-html="detail.shangjiagonggao"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.shangjiamingcheng
			bannerList.value = res.data.data.shangjiatupian?res.data.data.shangjiatupian.split(','):[]
