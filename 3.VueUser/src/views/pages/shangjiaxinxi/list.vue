	...
	<div class="app-contain" :style='{"padding":"0 10%","margin":"0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('shangjiaxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.shangjiatupian && item.shangjiatupian.substr(0,4)=='http'" @click.stop="preViewClick(item.shangjiatupian)">
							<el-image class="data_img" :src="item.shangjiatupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.shangjiatupian.split(',')[0])">
							<el-image class="data_img" :src="item.shangjiatupian?$config.url + item.shangjiatupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title3">
								商家名称：{{item.shangjiamingcheng}}
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/shangjiaxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('shangjiaxinxiDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
