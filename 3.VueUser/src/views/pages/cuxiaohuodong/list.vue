	...
	<div class="app-contain" :style='{"padding":"0 10%","margin":"0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('cuxiaohuodong','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="优惠方式" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.youhuifangshi}}
							</template>
						</el-table-column>
						<el-table-column label="促销条件" :resizable='true' align="left" header-align="left">
							<template #default="scope">
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/cuxiaohuodongAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('cuxiaohuodongDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
