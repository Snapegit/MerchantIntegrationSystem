		...
		<div class="app-contain">
			<div class="list_search_view">
				...
				<div class="btn_view">
					...
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('caipinxinxi','库存统计')">库存统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('caipinxinxi','评价')" type="success" @click="pingjiaxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">评价</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var kucunEchart1 = echarts.init(document.getElementById("kucunEchart1"),'macarons');
			context?.$http({
				url: `caipinxinxi/value/caipinmingcheng/kucun`,
				method: 'get'
			}).then(res=>{
    ...
    const pingjiaxinxiCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			pingjiaxinxiFormModelRef.value.init(row.id,'cross','评价',row,'caipinxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
